"""Diarize command — thin CLI wrapper."""

from __future__ import annotations

from pathlib import Path
from typing import Optional

import typer

from pyannote_cli import app
from pyannote_cli.backend import load_pipeline
from pyannote_cli.commands.diarize import run_diarize, format_rttm, format_json, format_txt


@app.command()
def diarize(
    file: Path = typer.Argument(..., help="Audio file to diarize"),
    output: Optional[Path] = typer.Option(None, "--output", "-o", help="Output file (default: stdout)"),
    format: str = typer.Option("rttm", "--format", "-f", help="Output format: rttm, json, txt"),
    num_speakers: Optional[int] = typer.Option(None, help="Exact number of speakers"),
    min_speakers: Optional[int] = typer.Option(None, help="Minimum number of speakers"),
    max_speakers: Optional[int] = typer.Option(None, help="Maximum number of speakers"),
    device: str = typer.Option("auto", help="Device: cpu, cuda, mps, auto"),
    token: Optional[str] = typer.Option(None, help="HuggingFace token (default: HF_TOKEN env)"),
) -> None:
    """Run speaker diarization on an audio file."""
    if not file.exists():
        typer.echo(f"ERROR: File not found: {file}", err=True)
        raise typer.Exit(code=1)

    pipeline = load_pipeline(
        "pyannote/speaker-diarization-community-1",
        token=token,
        device=device,
    )

    result = run_diarize(
        pipeline, file,
        num_speakers=num_speakers,
        min_speakers=min_speakers,
        max_speakers=max_speakers,
    )

    formatters = {"rttm": format_rttm, "json": format_json, "txt": format_txt}
    formatter = formatters.get(format)
    if formatter is None:
        typer.echo(f"ERROR: Unknown format '{format}'. Use: rttm, json, txt", err=True)
        raise typer.Exit(code=1)

    if format == "rttm":
        text = formatter(result, filename=file.stem)
    else:
        text = formatter(result)

    if output:
        output.parent.mkdir(parents=True, exist_ok=True)
        output.write_text(text + "\n")
        typer.echo(f"Saved to {output}")
    else:
        typer.echo(text)
