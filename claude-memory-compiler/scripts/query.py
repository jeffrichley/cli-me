...
Usage:
    uv run python query.py "How should I handle interactive prompts in CLI wrappers?"
    uv run python query.py "What patterns work for parallel agent research?" --file-back
"""
...
1. Create a Q&A article at {QA_DIR}/ with the filename being a slugified version
   of the question (e.g., knowledge/qa/how-to-handle-interactive-prompts.md)
...
5. Cite your sources using [[wikilinks]] (e.g., [[concepts/interactive-prompt-suppression]])
...
def main():
    parser = argparse.ArgumentParser(description="Query the cli-me knowledge base")
