...
Two-tier routing: the flush prompt distinguishes process knowledge (how to
build skills) from tool-specific knowledge (gotchas for a particular tool).
Both go into the daily log, but tagged differently so compile.py can route
them to the right wiki tier.
...
This is the cli-me project — a framework for building Claude Code skills that wrap
GUI/CLI software. Sessions typically involve building new skills, improving the build
process, running adversarial reviews, or doing SWOT analyses.

## Two-Tier Knowledge Routing

Tag each lesson learned with its tier:

- **[PROCESS]** — Knowledge about HOW to build skills. Patterns, architecture decisions,
  testing strategies, review techniques, agent coordination, meta-skill improvements.
  These apply to ALL future skill builds.
  Examples: "QA before implementation", "fresh context for reviewers", "parallel agents
  for research", "thin wrapper + logic layer".

- **[TOOL:<name>]** — Knowledge specific to a particular tool being wrapped (e.g.,
  [TOOL:ffmpeg], [TOOL:demucs], [TOOL:yt-dlp]). Flags, gotchas, API quirks, version
  differences. These apply only to that tool's skill.
  Examples: "[TOOL:ffmpeg] -y flag prevents interactive hangs", "[TOOL:demucs] GPU
  detection must probe the tool's Python not the wrapper's".
...
**Lessons Learned:**
- [PROCESS] or [TOOL:<name>] tag, then the lesson
- [PROCESS] or [TOOL:<name>] tag, then the lesson
